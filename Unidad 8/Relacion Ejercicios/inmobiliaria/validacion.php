...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (count($errores)==0){
        ...
        //Abre la base de datos y añade los datos.
        $db=conectWithDB("lindavista","root","");
        $sqlInsertPropiedades=$db->prepare("INSERT INTO Propiedades (tipo, zona, direccion, NumeroDeDormitorios, precio, tamano, UrlFoto, Observaciones) VALUES (?,?,?,?,?,?,?,?)");
        $sqlInsertPropiedades->bindValue(1,$tipoVivienda,PDO::PARAM_STR);
        $sqlInsertPropiedades->bindValue(2,$zona,PDO::PARAM_STR);
        $sqlInsertPropiedades->bindValue(3,$direccion,PDO::PARAM_STR);
        $sqlInsertPropiedades->bindValue(4,$numeroDormitorios,PDO::PARAM_INT);
        $sqlInsertPropiedades->bindValue(5,$precio,PDO::PARAM_INT);
        $sqlInsertPropiedades->bindValue(6,$metrosCuadrados,PDO::PARAM_INT);
        $sqlInsertPropiedades->bindValue(7,$nombreFoto,PDO::PARAM_STR);
        $sqlInsertPropiedades->bindValue(8,$observaciones,PDO::PARAM_STR);
        $sqlInsertPropiedades->execute();
        if ($extras!=null){
            $propiedadID=$db->lastInsertId();
            $sql=$db->prepare("INSERT INTO Extras (PropiedadID, NombreExtra) VALUES (?,?)");
            foreach ($extras as $extra){
                $sql->bindValue(1,$propiedadID,PDO::PARAM_INT);
                $sql->bindValue(2,$extra,PDO::PARAM_STR);
                $sql->execute();
            }
        }
    }
...
function conectWithDB($database,$username,$pw): ?PDO{
